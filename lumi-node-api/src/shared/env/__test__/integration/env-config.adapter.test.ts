...
describe('EnvConfig integration tests', () => {
  ...
  it('should return the enviroment DATABASE_NAME', async () => {
    ...
    expect(result).toStrictEqual('db-test');
  });

  it('should return the enviroment PDF_SERVICE_URL', async () => {
    const result = sut.getPDFServiceUrl();

    expect(result).toStrictEqual('http://test');
  });
});
